...
export class EmbedService extends BaseService {
    ...
    async createConfig(
        ...
        // Validate that at least one dashboard or chart is specified
        const dashboardUuids = data.dashboardUuids || [];
        const chartUuids = data.chartUuids || [];
        if (dashboardUuids.length === 0 && chartUuids.length === 0) {
    ...
    async updateConfig(
        ...
        // Throw error only if ALL conditions are true:
        // - not allowing all dashboards AND no dashboards specified
        // - not allowing all charts AND no charts specified
        if (
            !allowAllDashboards &&
            dashboardUuids.length === 0 &&
            !allowAllCharts &&
            (!chartUuids || chartUuids.length === 0)
        ) {
            throw new ParameterError(
                'At least one dashboard or chart must be specified for embedding',
            );
    ...
    async getChartUuidFromJwt(
        ...
        // Try to get the chart by UUID or slug
        try {
            const chart = await this.savedChartModel.get(contentId);

            // Verify the chart belongs to the correct project
            ...
            throw new NotFoundError(`Chart ${contentId} not found`);
        }
    }
